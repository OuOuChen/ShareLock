...
+ (YJSelectionView *_Nonnull)showWithTitle:(NSString *_Nonnull)title options:(NSArray *_Nonnull)optionsArray singleSelection:(BOOL)selection delegate:(id _Nonnull)delegate inView:(UIView *)inview completionHandler:(CompleteSelection _Nonnull)handler{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id object = self.optionsArray[indexPath.row];
    if ([object isKindOfClass:[COViewShareModel class]]) {
        COViewShareModel *model = self.optionsArray[indexPath.row];
        cell.descriptionLabel.text = [NSString stringWithFormat:@"车位锁编码%@",model.lock_id];
    }else{
        cell.descriptionLabel.text = [NSString stringWithFormat:@"%@",object];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isSingleSelection) {
        ...
        self.completeSelection(indexPath.row, self.optionsArray);
        [self closeView:self];
    } else {
