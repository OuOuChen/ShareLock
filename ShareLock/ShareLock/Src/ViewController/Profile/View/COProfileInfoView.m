...
@implementation COProfileInfoView
{
...
- (void)layoutSubviews
{
    ...
    if (self.style & 0x100)
    {
        CGFloat rightIconWidth  = self.size.height-30;
        CGFloat rightIconHeight = self.size.height-30;
        if (rightTmpView)
        {
        ...
        self.rightIcon.layer.cornerRadius = rightIconWidth/2;
...
- (UIImageView *)rightIndicator {
    if (_rightIndicator == nil) {
        _rightIndicator = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"more_icon"]];
    }
