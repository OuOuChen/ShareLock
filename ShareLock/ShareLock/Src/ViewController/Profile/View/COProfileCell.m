...
@implementation COProfileCell

+ (instancetype)cellWithTableView:(UITableView *)tableView {
    // 创建cell
    static NSString *ID = @"COProfileCell";
    COProfileCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    
    if (cell == nil) {
        cell = [[COProfileCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
    }
...
- (void)setupSubviews {
    ...
    arrowView.image = [UIImage imageNamed:@"more_icon"];
...
- (void)layoutSubviews {
    ...
    /** 4.箭头 */
    CGFloat arrowViewW = self.arrowView.image.size.width;
    CGFloat arrowViewX = contentW - 15 - arrowViewW;
    CGFloat arrowViewH = self.arrowView.image.size.height;
...
- (void)setItem:(COProfileItemModel *)item {
    _item = item;
